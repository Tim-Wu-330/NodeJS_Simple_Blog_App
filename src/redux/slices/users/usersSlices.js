...
export const unfollowUserAction = createAsyncThunk(
  ...
  async (unfollowId, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      ...
      // console.log(unfollowId, userAuth);
      return data;
    } catch (error) {
      if (!error?.response) {
        throw error;
...
export const uploadProfilePhototAction = createAsyncThunk(
  ...
  async (userImg, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      ...
      //userRoutes.put("/profilephoto-upload", authMiddleware, photoUpload.single("image"), profilePhotoResize profilePhotoUploadCtrl) from back-end
      return data;
    } catch (error) {
      if (!error?.response) throw error;
      return rejectWithValue(error?.response?.data);
...
const usersSlices = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(followUserAction.pending, (state, action) => {
      ...
      state.followedAppErr = undefined;
    ...
    builder.addCase(followUserAction.rejected, (state, action) => {
      state.followLoading = false;
      state.followedAppErr = action?.payload?.message;
